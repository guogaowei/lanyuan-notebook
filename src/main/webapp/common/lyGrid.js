	...
	lyGrid = (function(params) {
		...
		var cBodyTable = function(divid, jsonData) {
			...
			$.each(json, function(d) {
				...
				chkbox.setAttribute("pid", _getValueByName(json[d], "parentId"));
				// ******** 树的上下移动需要
					...
					if (!column[o].hide || column[o].hide == undefined) {
						...
						if (l_tree.tree) {
							if (l_tree.name == clm) {
								...
								divtree.setAttribute("style", "padding-left:5px;text-align:" + column[o].align + ";");
						...
						if (!column[o].hide || column[o].hide == undefined) {
							...
							if (l_tree.tree) {
								if (l_tree.name == column[o].colkey) {
									var divtree = document.createElement("div");
									divtree.className = "ly_tree";
									divtree.setAttribute("style", "padding-left:5px;text-align:" + column[o].align + ";margin-left: " + nb + "px;");
									td_o.appendChild(divtree);
									var divspan = document.createElement("span");
									divspan.className = "l_test";
									divspan.setAttribute("style", "line-height:" + conf.tbodyHeight + ";");
